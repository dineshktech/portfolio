...
fun Footer() {
	...
	Footer({
		...
		Div({
			...
			Button({
				...
				onClick {
					...
					window.open("mailto:manidinesh89@gmail.com?subject=$subject&body=$body", "_blank")
				}
			}) {
				Text("Send message")
		...
		Div({
			...
			val cvPath = "${window.location.origin}/resume.pdf"
			
			A(href = cvPath, {
				target(ATarget.Blank)
				attr("download", "Dinesh Kumar M - Resume.pdf")
...
object FooterStyle : StyleSheet() {
	...
	val footerContactInputs by style {
		...
		gridTemplateColumns(utils.repeat(2, 1.fr))
		gridTemplateRows(utils.repeat(4, 1.fr))
		gap(1.cssRem)
	...
	val footerCVButton by style {
		val buttonBackgroundColor = Color("#32AC66")
